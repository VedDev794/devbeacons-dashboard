...
export function WorkflowList({ repoName, onSelectWorkflow }: WorkflowListProps) {
  const [workflows, setWorkflows] = useState<Workflow[]>([]);
  const [stats, setStats] = useState<WorkflowStats>({ success: 0, failure: 0, progress: 0, cancel: 0 });
  ...
  useEffect(() => {
    async function loadWorkflows() {
      ...
      try {
        setLoading(true);
        const { workflows, stats } = await fetchWorkflows(accessToken, ownerName, repoName);
  ...
  if (workflows.length === 0) {
    return (
      <div className="px-4 py-6 text-center">
        ...
        <p className="text-sm text-muted-foreground">No workflows found for this repository.</p>
      </div>
    );
  }
